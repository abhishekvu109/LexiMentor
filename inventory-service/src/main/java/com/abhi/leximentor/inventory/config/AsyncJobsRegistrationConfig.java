...
public class AsyncJobsRegistrationConfig {
    ...
    public AsyncJobRegistration inventoryAsyncJobRegistration(
        ...
        return jobClient -> {
            ...
            jobClient.register(
                JobDefinition.builder("leximentor.drill.challenge.evaluate", DrillChallengeEvaluationJobPayload.class, ctx -> {
                        DrillChallengeEvaluationJobPayload payload = ctx.payload();
                        DrillChallenge challenge = drillChallengeRepository.findByRefId(payload.getChallengeRefId());
                        List<DrillChallengeScoresDTO> scores = drillChallengeScoreService.getByDrillChallengeId(challenge);
