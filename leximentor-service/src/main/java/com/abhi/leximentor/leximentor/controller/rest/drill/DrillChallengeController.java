...
public class DrillChallengeController {
    ...
    public ResponseEntity<RestApiResponse> getAllChallengesByDrillRefId(@PathVariable String drillRefId, @RequestParam String username) {
        ...
        List<ChallengeDTO> ChallengeDTOList = drillChallengeService.getChallengesByDrillRefIdAndUsername(Long.parseLong(drillRefId), username);
        return ChallengeDTOList != null ? ResponseEntityBuilder.getBuilder(HttpStatus.OK).successResponse(ApplicationConstants.REQUEST_SUCCESS_DESCRIPTION, ChallengeDTOList) : ResponseEntityBuilder.getBuilder(HttpStatus.INTERNAL_SERVER_ERROR).errorResponse(ApplicationConstants.REQUEST_FAILURE_DESCRIPTION, "Unable to retrieve challenges");
    }

    @GetMapping(value = "/api/leximentor/drill/metadata/challenges/challenge/evaluators", produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    @DeleteMapping(value = "/api/leximentor/drill/metadata/challenges/{drillRefId}", produces = MediaType.APPLICATION_JSON_VALUE)
